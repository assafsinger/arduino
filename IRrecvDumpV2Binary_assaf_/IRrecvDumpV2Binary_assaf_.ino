...
String resultToSourceCodeBinary(const decode_results * const results) {
  ...
  for (uint16_t i = 1; i < results->rawlen; i++) {
    ...
    if (usecs <= zeroThreshold)
    {
      output+=F("0");
    } else if (usecs <= oneThreshold){
      output+=F("1");
    } else {
      output+=F("?");
    }
...
void loop() {
  ...
  if (irrecv.decode(&results)) {
    ...
    yield();  // Feed the WDT (again)rawData
#endif  // LEGACY_TIMING_INFO
    // Output the results as source code
    Serial.println(resultToSourceCode(&results));
    Serial.println(resultToSourceCodeBinary(&results));
