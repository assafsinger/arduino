...
uint32_t ESP8266::recv(uint8_t *buffer, uint32_t buffer_size, uint32_t timeout, uint32_t data_indx_strt)
{
    return recvPkg(buffer, buffer_size, NULL, timeout, NULL, data_indx_strt);
}

uint32_t ESP8266::recv(uint8_t mux_id, uint8_t *buffer, uint32_t buffer_size, uint32_t timeout, uint32_t data_indx_strt)
{
    uint8_t id;
    uint32_t ret;
    ret = recvPkg(buffer, buffer_size, NULL, timeout, &id,data_indx_strt);
    if (ret > 0 && id == mux_id) {
        return ret;
    }
    return 0;
...
{
    return recvPkg(buffer, buffer_size, NULL, timeout, coming_mux_id,0);
}
...
uint32_t ESP8266::recvPkg(uint8_t *buffer, uint32_t buffer_size, uint32_t *data_len, uint32_t timeout, uint8_t *coming_mux_id, uint32_t data_indx_strt)
{
    bool debug = true;
    String data;
    char a;
...
//        Serial.print("has data\r\n");
        ...
        while (millis() - start < 3000 && !done)
        {
            while(m_puart->available() > 0) {
                if (count == len || i == buffer_size) {
                    Serial.print(F("breaking"));
                    done = true;
                    break;
                }
                a = m_puart->read();
                if (++count > data_indx_strt){
                    if (has_data)
                        buffer[i++] = a;
                    Serial.print(a);
                }
...
bool ESP8266::eATGMR(String &version)
{
    ...
    return recvFindAndFilter(OK, F("\r\r\n"), F("\r\n\r\nOK"), version);
}
...
bool ESP8266::qATCWMODE(uint8_t *mode)
{
    ...
    ret = recvFindAndFilter(OK, F("+CWMODE:"), F("\r\n\r\nOK"), str_mode);
    if (ret) {
...
bool ESP8266::sATCWMODE(uint8_t mode)
{
    ...
    data = recvString(OK, F("no change"));
    if (data.indexOf(OK) != -1 || data.indexOf(F("no change")) != -1) {
        return true;
    }
    return false;
}
...
bool ESP8266::sATCWJAP(String ssid, String pwd)
{
    ...
    data = recvString(OK, F("FAIL"), 10000);
...
bool ESP8266::sATCWSAP(String ssid, String pwd, uint8_t chl, uint8_t ecn)
{
    ...
    data = recvString(OK, F("ERROR"), 5000);
    if (data.indexOf(OK) != -1) {
        return true;
    }
    return false;
...
bool ESP8266::sATCIPSTARTSingle(String type, String addr, uint32_t port)
{
    ...
    data = recvString(OK, F("ERROR"), F("ALREADY CONNECT"), 10000);
    if (data.indexOf(OK) != -1 || data.indexOf(F("ALREADY CONNECT")) != -1) {
        return true;
    }
    return false;
}
bool ESP8266::sATCIPSTARTMultiple(uint8_t mux_id, String type, String addr, uint32_t port)
{
    ...
    data = recvString(OK, "ERROR", F("ALREADY CONNECT"), 10000);
    if (data.indexOf(OK) != -1 || data.indexOf(F("ALREADY CONNECT")) != -1) {
        return true;
    }
    return false;
...
bool ESP8266::sATCIPCLOSEMulitple(uint8_t mux_id)
{
    ...
    data = recvString(OK, F("link is not"), 5000);
    if (data.indexOf(OK) != -1 || data.indexOf(F("link is not")) != -1) {
        return true;
    }
    return false;
}
...
bool ESP8266::sATCIPMUX(uint8_t mode)
{
    ...
    data = recvString(OK, F("Link is builded"));
    if (data.indexOf(OK) != -1) {
        return true;
    }
    return false;
