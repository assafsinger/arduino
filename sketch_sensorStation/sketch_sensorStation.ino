...
//    char msg1[7] = {'h','e','l','l','o',' ','#'};
      ...
      Serial.println("Original length:" + strlen(msg));

      int counter = 0;
      while (true) {
        int len = strlen(&msg[counter]);
        if (len>27){
          char buffer[27];
          memcpy(buffer, &msg[counter], 27);
          buffer[27] = '\0';
          Serial.print("Sending part:");
          Serial.println(buffer);
          vw_send((uint8_t *)buffer, strlen(buffer));
          counter+=27;
        } else { //msg is shorter than 27
          Serial.print("Sending part:");
          Serial.println(&msg[counter]);
          vw_send((uint8_t *)&msg[counter], strlen(&msg[counter]));
          break;
        }
      }
...
void loop() {
 ...
 //build the message
  ...
  if (isnan(t) || isnan(h)) {
    sprintf(message, "ID:%d:ER:ERROR\0", MYID);  //millis provides a stamp for deduping if signal is repeated
    ...
    if (UNIT == 0 ){
      sprintf(message, "ID:%d:TF:%s:RH:%s\0", MYID, temp, hum);  //millis provides a stamp for deduping if signal is repeated
    }
    else { //Celsius
      sprintf(message, "ID:%d:TC:%s:RH:%s\0", MYID, temp, hum);  //millis provides a stamp for deduping if signal is repeated
    }
